  ...
  const aggregate = Invoice.aggregate([
    ...
    // ── MAIN CALCULATION ──────────────────────────────────────────────────
    {
      $addFields: {
        ...
        // ── discountTotal ────────────────────────────────────────────────
        // Frontend forEach:
        //   let discount = item.discount * qty
                          ...
                          in: {
                            $max: [
                              0,
                              {
                                $cond: [
                                  '$$isIncl',
                                  // inclusive: strip GST from (price - disc)
                                  {
                                    $divide: [
        ...
        // ── gstTotal ─────────────────────────────────────────────────────
        // gstAmount = itemTaxable × (gstRate / 100)
        // itemTaxable is already excl. GST in both modes
        gstTotal: {
          $round: [
            {
              $sum: {
